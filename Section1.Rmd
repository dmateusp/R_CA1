...
### CUST_MOS
```{r}
ggplot(data=phones ,aes(phones$CUST_MOS)) + geom_histogram(binwidth=6) + scale_x_continuous(breaks = seq(0,60,5)) + scale_y_continuous(breaks = seq(0,1000,50))
```
### NUM_LINES
```{r}
...
### OUTLIERS
```{r}
ggplot(data=phones ,aes(x=phones$TOT_MINUTES_USAGE, y=phones$MINUTES_CURR_MONTH)) + geom_point()
```
